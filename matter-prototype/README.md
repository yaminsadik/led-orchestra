...
- Hardware: ESP32-C6 LED nodes; the hub is the Espressif ESP Thread BR board
  (ESP32-S3 host + ESP32-H2 RCP). All-Espressif, not all-C6.
...
- Matter controller/commissioner: the controller node — the local source of truth
  ...
  Router and evolves into the **S3+H2 one-board hub** (Matter controller +
  esp-thread-br host on the ESP32-S3, ESP32-H2 RCP) under the amended,
  validation-gated decision. See
...
| `controller-node/` | ESP-IDF app for the dedicated Matter controller node (evolves into the S3+H2 hub). |
...
| `s3-h2-hub-validation/` | S3+H2 one-board hub runbooks + committed config (primary hub validation; Stages A-F). |
...
5. Bring up the border router. **Primary path:** the S3+H2 one-board hub (S3
   esp-thread-br host + H2 RCP) — see
   [`s3-h2-hub-validation/`](s3-h2-hub-validation/) (Stages A-C). The proven
   all-C6 split (separate BR-host C6 + RCP C6) is the fallback and the historical
   Stage 0 proof: [`stage0-br-validation/README.md`](stage0-br-validation/README.md).
...
OpenThread Border Router (confirmed on hardware). The next work is the S3+H2
one-board hub validation (Stages A-F) that selects the S3+H2 hub, the all-C6
split, or Pi — see
