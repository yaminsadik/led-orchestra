...
esp_err_t EspOtaProvider::RegisterBdxHandler()
{
    return RegisterBdxHandler(chip::Server::GetInstance().GetExchangeManager());
}

esp_err_t EspOtaProvider::RegisterBdxHandler(chip::Messaging::ExchangeManager &exchangeManager)
{
    CHIP_ERROR err = exchangeManager.RegisterUnsolicitedMessageHandlerForType(chip::bdx::MessageType::ReceiveInit,
                                                                              &mOtaBdxSender);
    if (err != CHIP_NO_ERROR) {
        ESP_LOGE(TAG, "failed to register BDX ReceiveInit handler: %" CHIP_ERROR_FORMAT, err.Format());
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "registered BDX ReceiveInit handler %p on exchange manager %p", &mOtaBdxSender, &exchangeManager);
    return ESP_OK;
}
...
{
    // Pre-create the entry if it doesn't exist so lo-ota-enable works before
    // the first QueryImage arrives. CreateOtaRequestorEntry is idempotent: if
    // the entry already exists it leaves it untouched, so this is safe to call
    // unconditionally when a specific node id is given.
    if (nodeId.GetNodeId() != chip::kUndefinedNodeId) {
        if (CreateOtaRequestorEntry(nodeId) != ESP_OK) {
