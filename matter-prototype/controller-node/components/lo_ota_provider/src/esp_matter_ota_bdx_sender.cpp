...
void OtaBdxSender::HandleTransferSessionOutput(TransferSession::OutputEvent &event)
{
    ...
    case TransferSession::OutputEventType::kInitReceived: {
        ...
        // Establish http connection. LED Orchestra offline fork: the image is
        // streamed from a hub-local HTTP endpoint on the control LAN (operator
        // laptop now, Kubernetes-served later), so we let the URL scheme pick the
        // transport (HTTP_TRANSPORT_OVER_TCP) instead of forcing TLS against the
        // public CA bundle. An https:// ota_url still works — esp_http_client
        // selects TLS from the scheme — but the offline default needs no certs.
